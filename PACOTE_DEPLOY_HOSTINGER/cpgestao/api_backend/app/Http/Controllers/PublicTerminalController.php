...
class PublicTerminalController extends Controller
{
    ...
    private function validateDevice($slug, $uid = null)
    {
        ...
        if ($tenant->status !== 'active') {
            abort(403, 'Esta loja está temporariamente inativa.');
        }

        if ($tenant->plan_expires_at && \Carbon\Carbon::parse($tenant->plan_expires_at)->isPast()) {
            abort(403, 'O plano desta loja expirou.');
        }
    ...
    public function getInfo($slug, $uid)
    {
        ...
        return ApiResponse::ok([
            ...
            'vip_unlinked' => ($device ? $device->type : 'public_link' === 'premium' && !$device->linked_customer_id),
            'message' => ($device ? $device->type : 'public_link' === 'premium' && !$device->linked_customer_id) ? 'Cartão não vinculado. Procure o balcão.' : null
        ]);
    }
    ...
    public function redeem(Request $request, $slug, $uid)
    {
        ...
        return DB::transaction(function () use ($request, $slug, $uid) {
            ...
            // Lógica "Próxima Visita": O cliente resgata e já ganha os pontos da visita atual
            $customer->points_balance = ($customer->points_balance - $goal) + $pointsToAdd + $bonus;
