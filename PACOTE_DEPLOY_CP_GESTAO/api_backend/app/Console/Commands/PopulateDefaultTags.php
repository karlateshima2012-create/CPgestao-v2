...
class PopulateDefaultTags extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tags:populate-defaults';
     ...
     * @var string
     */
    protected $description = 'Populate default intelligent tags for all existing tenants';
    ...
    {
        $tenants = Tenant::all();
        $this->info("Found " . $tenants->count() . " tenants. Populating default tags...");
        ...
        foreach ($tenants as $tenant) {
            foreach ($defaultTags as $tagData) {
                ...
                $existing = TenantTag::where('tenant_id', $tenant->id)
                    ->where('name', $tagData['name'])
                    ->first();

                if (!$existing) {
                    TenantTag::create(array_merge($tagData, ['tenant_id' => $tenant->id]));
                }
            }
            $this->info("Tags populated for tenant: {$tenant->name}");
        }

        $this->info('Finished populating default tags for all tenants.');
    }
}
